...
# 1. Para a API (Inglês)
LIVROS_INGLES = {
    "Deuteronômio": "Deuteronomy", "Hebreus": "Hebrews", "Gênesis": "Genesis", "Números": "Numbers",
...
# 2. Para o Link de Backup (Siglas em Português)
SIGLAS_BIBLIAONLINE = {
    "Deuteronômio": "dt", "Hebreus": "hb", "Gênesis": "gn", "Números": "nm",
    "Mateus": "mt", "Ageu": "ag", "João": "jo", "Jeremias": "jr",
    "1 Coríntios": "1co", "Apocalipse": "ap", "Provérbios": "pv",
    "2 Timóteo": "2tm", "Isaías": "is", "Tiago": "tg", "Salmos": "sl",
    "Romanos": "rm", "Ezequiel": "ez", "Lucas": "lc", "Gálatas": "gl",
    "2 Crônicas": "2cr", "1 João": "1jo", "Filipenses": "fp",
    "Colossenses": "cl", "Efésios": "ef"
}

def buscar_texto_biblico(referencia_pt):
    try:
        # Separação do nome do livro e capítulo
        ...
        # Define capitulo_versiculo para a API
...
def gerar_link_backup(referencia_pt):
    """Gera o link correto para o site Bíblia Online (ex: acf/ef/1)"""
    try:
        partes = referencia_pt.split()
        if partes[0].isdigit():
            livro_pt = f"{partes[0]} {partes[1]}"
        ...
        # Pega apenas o número do capítulo (antes dos dois pontos)
        if ":" in restante:
            capitulo = restante.split(":")[0]
        else:
            capitulo = restante

        sigla = SIGLAS_BIBLIAONLINE.get(livro_pt)
        
        if sigla:
            return f"https://www.bibliaonline.com.br/acf/{sigla}/{capitulo}"
        else:
            # Fallback genérico caso falte a sigla
            return f"https://www.bibliaonline.com.br/acf/{referencia_pt.replace(' ', '/').replace(':', '/')}"
            
    except:
        return "https://www.bibliaonline.com.br/acf"
...
def enviar_mensagem():
    ...
    if hoje in calendario:
        ...
        if texto_biblico:
            ...
            # Gera o link corrigido com a sigla
            link_backup = gerar_link_backup(ref)
